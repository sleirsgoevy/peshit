...
def compile_arm(c_code, asm_code, link_addr):
    with tempfile.TemporaryDirectory() as file:
        ...
        assert not p.wait()
        p = subprocess.Popen((CC_ARM_HOST+'gcc', '-x', 'assembler', '-', '-nostdlib', '-static', '-Ttext='+hex(link_addr), '-o', file+'/out.elf'), stdin=subprocess.PIPE, stdout=subprocess.PIPE, encoding='utf-8')
